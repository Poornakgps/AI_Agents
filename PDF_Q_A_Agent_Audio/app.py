...
model = whisper.load_model("base.en")  # You can choose different model sizes (base, small, medium, large)

# Speech-to-text conversion function using Whisper
def speech_to_text(audio_file):
    try:
        # Load audio file and transcribe using Whisper
        audio = whisper.load_audio(audio_file)
        audio = whisper.pad_or_trim(audio)
        result = model.transcribe(audio)
        return result["text"]
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error in speech recognition: {str(e)}")
...
# Global QA system object
qa_system = None

# API endpoint to process speech or text query
@app.post("/process_query/") 
async def process_query(audio: UploadFile = File(None), question: str = None):
    try:
        # If audio file is provided, convert it to text
        if audio:
            audio_file = BytesIO(await audio.read())
            query_text = speech_to_text(audio_file)
        elif question:
            query_text = question
        else:
            raise HTTPException(status_code=400, detail="No query provided (either audio or text).")

        # Now, process the query text with the QA system
        ...
        answer = qa_system.ask_question(query_text, qa_system.vector_store, qa_system.chunks)
...
async def pdf_status():
    ...
    if qa_system and qa_system.vector_store is not None:
